...
if($nonce == $_SESSION['nonce']){
	...
	$plays_query = "SELECT * FROM $plays_table WHERE day = '$day' AND month = '$month' AND year = '$year' ORDER BY score DESC";
	...
	if($plays_res){
		...
		if($num_of_plays > 0){
			...
			$leaders_html = "<p>There are no scores recorded for today, be the first to put your name on this leaderboard.</p>";
		}
